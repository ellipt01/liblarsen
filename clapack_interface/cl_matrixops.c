...
cl_vector_mean (const cl_vector *v)
{
	int	i;
	double	sum;
	if (cl_vector_is_empty (v)) cl_error ("cl_vector_mean", "vector *v is empty");
	/* x = sum x / N */
...
cl_vector_sub (cl_vector *y, const cl_vector *x)
{
	...
	if (x->size != y->size) cl_error ("cl_vector_sub", "vector size are not match");
	/* y = y - x */
...
cl_vector_axpy (const double alpha, const cl_vector *x, cl_vector *y)
{
	...
	if (x->size != y->size) cl_error ("cl_vector_axpy", "vector size are not match");
	/* y = y + alpha * x */
...
{
	if (cl_vector_is_empty (v1)) cl_error ("cl_vector_dot_vector", "first vector is empty");
	if (cl_vector_is_empty (v2)) cl_error ("cl_vector_dot_vector", "second vector is empty");
...
cl_matrix_dot_vector (const cl_matrix *a, const cl_vector *v)
{
	...
	if (a->size2 != v->size) cl_error ("cl_matrix_dot_vector", "vector and matrix size are not match");
...
cl_matrix_transpose_dot_vector (const cl_matrix *a, const cl_vector *v)
{
	...
	if (a->size1 != v->size) cl_error ("cl_matrix_transpose_dot_vector", "vector and matrix size are not match");
