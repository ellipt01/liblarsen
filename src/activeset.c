...
{
	if (l->sizeA >= l->lreg->x->n) return false;
...
complementA (larsen *l)
{
	...
	/* Ac : complement of A */
	for (i = 0, j = 0, k = 0; i < l->lreg->x->n && k < n; i++) {
...
update_activeset (larsen *l)
{
	...
	if (!check_index (l->lreg->x->n, l->oper.column_of_X)) return false;
	if (!check_index (l->lreg->x->n, l->oper.index_of_A)) return false;
