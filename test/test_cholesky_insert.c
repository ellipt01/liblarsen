...
test_cholesky_insert (void)
{
	...
	for (i = 0; i < size; i++) {
		int		j;
		for (j = 0; j < i; j++) la[LINREG_INDEX_OF_MATRIX(i, j, size)] = 0.;
	}
	...
	u = (double *) malloc (size * sizeof (double));
	{
		...
		for (i = 0; i < size - 1; i++) {
			for (j = 0; j < size - 1; j++) {
				b[LINREG_INDEX_OF_MATRIX (i, j, size - 1)] = a[LINREG_INDEX_OF_MATRIX (i, j, size)];
			}
		}
		for (i = 0; i < size; i++) u[i] = a[LINREG_INDEX_OF_MATRIX (i, index, size)];
	}
	free (a);
	...
	for (i = 0; i < size; i++) {
		int		j;
		for (j = 0; j < i; j++) lb[LINREG_INDEX_OF_MATRIX(i, j, size)] = 0.;
	}
