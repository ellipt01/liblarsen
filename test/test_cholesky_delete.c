...
test_cholesky_delete (void)
{
	...
	a = (double *) malloc ((size - 1) * (size - 1) * sizeof (double));
	{
		...
		for (i = 0, m = 0; i < size; i++) {
			...
			for (j = 0, n = 0; j < size; j++) {
				if (j == index) continue;
				a[LINREG_INDEX_OF_MATRIX (m, n, size - 1)] = b[LINREG_INDEX_OF_MATRIX (i, j, size)];
				n++;
			}
			m++;
		}
	...
	for (i = 0; i < size - 1; i++) {
		int		j;
		for (j = 0; j < i; j++) la[LINREG_INDEX_OF_MATRIX(i, j, size - 1)] = 0.;
	}
	free (a);
	...
	for (i = 0; i < size - 1; i++) {
		int		j;
		for (j = 0; j < i; j++) lb[LINREG_INDEX_OF_MATRIX(i, j, size - 1)] = 0.;
	}
